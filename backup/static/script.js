...
async function updateDish() {
    ...
    const result = await apiRequest(`/menu/${encodeURIComponent(currentEditDish)}`, {
        method: 'PUT',
...
async function loadStats() {
    ...
    if (result.success) {
        ...
        const statsElement = document.getElementById('menuStats');
...
async function resetMenu() {
    if (!confirm('确定要重置菜单吗？这将恢复为默认菜单，当前菜单将备份。')) {
        return;
    }

    // 这里可以调用重置API，或者直接重新初始化
    showNotification('重置功能待实现', 'info');
}
